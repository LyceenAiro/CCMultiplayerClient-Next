...
const enDict: { [key: string]: string } = {
    ...
    unstuck: 'Unstuck',
    // ROUND 161 — unstuck mercy window: first use free, re-use within 60s pays fall damage
    mpUnstuckFree: 'Unstuck: no damage taken',
    mpUnstuckPaid: 'Unstuck again within 60s: water-fall damage taken',
    // ROUND 162 — progress wall (server blockedMaps): entry refused / kicked out
    mpWallBlocked: 'This area is not available yet - please wait for a future update',
    mpWallKicked: 'This area has been closed by the server - returning to the previous area',
    levelLabel: 'Lv ',
    expLabel: 'EXP ',
    hpLabel: 'HP ',
    ...
    loginRecent: 'Recent',
    // 1.78.x — account passwords + one-time reset codes
    loginPasswordPh: 'Password (leave empty if none)',
    loginForgot: 'Forgot password?',
    loginWrongPassword: 'Wrong password',
    resetTitle: 'Reset Password',
    resetDesc: 'Requesting a code generates it on the server ADMIN page — ask your server admin for it (valid 2 hours, one-time use).',
    resetRequest: 'Get Reset Code',
    resetCodePh: '6-character code',
    resetNewPwPh: 'New password (4-64 chars)',
    resetConfirm: 'Confirm Reset',
    resetBack: 'Back to Login',
    resetOk: 'Password reset — log in with the new password.',
    resetSending: 'Sending…',
    setPwTitle: 'Set Login Password',
    setPwMsg: 'This account has no password yet. To protect your online save, set a login password now — you must set one before playing.',
    setPwPh1: 'New password (4-64 chars)',
    setPwPh2: 'Confirm password',
    setPwSubmit: 'Set Password',
    setPwMismatch: 'The two passwords do not match',
    setPwWeak: 'Password must be 4-64 characters',
    setPwFailed: 'Failed to set password',
    ...
    // 1.77.x — level-regression save-anomaly guard (forced return-to-title dialog)
    saveLevelResetTitle: 'Save Anomaly Detected',
    saveLevelResetMsg: 'Your previous save was level {prev}, but this save would be level 1 — the upload was rejected as an anomaly and your cloud save was NOT overwritten. Return to the title screen and log in again to restore your progress.',
    ...
    // 1.77.x — admin resetMap + quest-instance regroup confirm
    adminResetMap: 'An admin reset this map\'s progress ({n} vars) and reloaded it',
    teleportQuestWarn: 'The teammate is on a quest-only map. Joining mid-quest saves its progress into YOUR save and can break that quest for you. Teleport anyway?',
    ...
    // 1.77.x — cutscene unstuck button (pause menu; label reuses 'unstuck')
    mpCsUnstuckNone: 'No stall detected — the scene is playing normally',
    mpCsUnstuckNudge: 'Unstuck attempt: cancelled the stalled action',
    mpCsUnstuckEnded: 'Unstuck: force-ended the stalled scene',
    mpCsUnstuckState: 'Unstuck: restored game control',
...
const zhDict: { [key: string]: string } = {
    ...
    unstuck: '脱离卡死',
    // ROUND 161 — 脱离卡死宽限窗口：首次免费，60秒内重复使用按落水掉血
    mpUnstuckFree: '已脱离卡死：本次无伤害',
    mpUnstuckPaid: '已脱离卡死：60秒内重复使用，承受落水伤害',
    // ROUND 162 — 进度墙（服务器 blockedMaps）：禁止进入 / 遣返提示
    mpWallBlocked: '该区域尚未开放，请等待后续更新',
    mpWallKicked: '该区域已被服务器关闭，正在遣返回上一区域',
    levelLabel: '等级 ',
    expLabel: '经验 ',
    hpLabel: 'HP ',
    ...
    loginRecent: '最近登录',
    // 1.78.x — 账号密码与一次性重置验证码
    loginPasswordPh: '密码（未设置可留空）',
    loginForgot: '忘记密码？',
    loginWrongPassword: '密码错误',
    resetTitle: '重置密码',
    resetDesc: '获取验证码后，服务端管理页面会生成一次性验证码（2 小时内有效），请联系服务器管理员索取。',
    resetRequest: '获取重置验证码',
    resetCodePh: '6 位验证码',
    resetNewPwPh: '新密码（4-64 位）',
    resetConfirm: '确认重置',
    resetBack: '返回登录',
    resetOk: '密码已重置，请使用新密码登录。',
    resetSending: '发送中…',
    setPwTitle: '设置登录密码',
    setPwMsg: '该账号尚未设置密码。为保护云端存档安全，请现在设置登录密码——设置后才能进入游戏。',
    setPwPh1: '新密码（4-64 位）',
    setPwPh2: '确认新密码',
    setPwSubmit: '设置密码',
    setPwMismatch: '两次输入的密码不一致',
    setPwWeak: '密码需为 4-64 个字符',
    setPwFailed: '设置密码失败',
    // 1.71.0 — 存档镜像回溯
    loginMirror: '镜像回溯',
    mirrorTitle: '存档镜像回溯',
    ...
    // 1.77.x — 等级回退存档异常校验（强制返回主界面弹窗）
    saveLevelResetTitle: '存档出现异常',
    saveLevelResetMsg: '检测到你上一次存档为 {prev} 级，但本次保存时等级为 1 级，判定为存档异常。本次存档已被拒绝，云端存档未受影响。请返回主界面重新登录，以恢复你的存档进度。',
    // 1.73.0 — 服务器管理调试命令
    ...
    // 1.77.x — 管理员重置地图进度 + 任务地图传送确认
    adminResetMap: '管理员重置了你当前地图的进度（清除了 {n} 个进度变量），地图已重载',
    teleportQuestWarn: '队友所在的地图是任务专属地图。若对方正在攻略任务，传送过去会把地图进度写入你的存档，可能导致该任务永久无法进行。仍要传送吗？',
    ...
    // 1.77.x — 剧情脱离卡死按钮（暂停菜单；按钮文字复用 'unstuck'）
    mpCsUnstuckNone: '当前未检测到卡死，剧情正在正常播放',
    mpCsUnstuckNudge: '已尝试脱离卡死：取消了卡住的行动',
    mpCsUnstuckEnded: '已脱离卡死：强制结束了卡住的剧情',
    mpCsUnstuckState: '已脱离卡死：恢复了游戏控制',
